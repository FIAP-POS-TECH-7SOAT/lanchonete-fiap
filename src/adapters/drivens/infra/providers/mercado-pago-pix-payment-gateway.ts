...
export class MercadoPagoPixPaymentGateway implements IPaymentGateway {
  async processPayment({
    ...
    order_id,
    customer,
  }: ProcessPaymentRequest): Promise<ProcessPaymentResponse> {
    try {
      const client = new MercadoPagoConfig({
        accessToken: env.PAYMENT_GATEWAY_ACCESS_TOKEN,
      });
      ...
      return {
        ...
        qr_code: paymentCreated.point_of_interaction?.transaction_data
          ?.qr_code as unknown as string,
        qr_code_base64: paymentCreated.point_of_interaction?.transaction_data
          ?.qr_code_base64 as unknown as string,
